...
color_aware_voxel_smooth_cuda = load(
        name='color_aware_voxel_smooth_cuda',
        sources=[
            os.path.join(parent_dir, path)
            for path in ['cuda/color_aware_voxel_smooth.cu', 'cuda/color_aware_voxel_smooth.cpp']],
        verbose=True)
...
class DirectVoxGO(torch.nn.Module):
    ...
    def color_aware_voxel_smooth_add_grad(self,weight,dense_mode):
        w = weight * self.world_size.max() / 128
        color_aware_voxel_smooth_cuda.color_aware_voxel_smooth_add_grad(self.k0.grid.detach(),self.density.grid,self.density.grid.grad,w,w,w,dense_mode,True)
    ...
    def forward(self, rays_o, rays_d, viewdirs, global_step=None, **render_kwargs):
        ...
        N_samples_id = N_samples[ray_id]
        s = (step_id+0.5) / N_samples_id
        # Ray marching
        ...
        if render_kwargs.get('render_depth', False) or render_kwargs.get('render_normal', False):
            depth = segment_coo(
                    src=(weights * step_id),
                    index=ray_id,
                    out=torch.zeros([N]),
...
def get_random_poses(train_poses, generate_poses='loaded', n_poses=20):
    ...
    if generate_poses == 'loaded':
        ...
        for index in range(n_poses):
            ...
            poses[index]=utils.interp3(pose1[:3,:4], pose2[:3,:4], pose3[:3,:4], s12, s3)
...
def get_training_rays_flatten(rgb_tr_ori, train_poses, HW, Ks, ndc, inverse_y, flip_x, flip_y):
    print('get_training_rays_flatten: start') # Here gets all the rays in training set...
