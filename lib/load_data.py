...
def load_data(args):
    ...
    if args.dataset_type == 'llff':
        ...
        images, render_poses, hwf, masks = load_dtu_data(args.datadir,args.mask_index,args.maskdir)
        print('Loaded DTU', images.shape, poses.shape, hwf, args.datadir)
        if args.mask_index is not None:
            i_test = np.array([i for i in args.mask_index])
        ...
        if args.mask_index is None:
            ...
            i_train = np.array([i for i in args.mask_index if
                        (i not in i_test and i not in i_val)])
